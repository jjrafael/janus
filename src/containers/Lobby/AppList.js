...
class AppList extends React.Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    {
      prevProps.user.details.id !== this.props.user.details.id && this.props.fetchApps(this.props.user.details.id)
    }
  }
    ...
    if (!!this.props.apps.appList.length && !this.props.apps.fetchAppsFailed && !this.props.apps.isFetchingApps) {
      return (
        <div className="section-wrapper">
          <h3>Applications</h3>
          <div className="apps-container">{this._renderAppList()}</div>
        </div>
      )
