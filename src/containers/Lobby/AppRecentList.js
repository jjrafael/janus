...
class AppRecentList extends React.Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    {
      prevProps.user.details.id !== this.props.user.details.id && this.props.fetchRecentApps(this.props.user.details.id)
    }
  }
  ...
  _renderAppRecentList() {
    ...
    return this.props.apps.appRecentList.map(app => {
      ...
      return (
        <div
          className="btn-launch btn-action"
    ...
    if (!!apps.appRecentList.length && !apps.fetchRecentAppsFailed && !apps.isFetchingRecentApps) {
      return (
        <div className="section-wrapper">
          ...
          <div className="apps-container">{this._renderAppRecentList()}</div>
        </div>
      )
    } else if (!apps.fetchRecentAppsFailed && apps.isFetchingRecentApps) {
      return (
        <div className="section-wrapper">
          <div className="loading-container tcenter">{this._renderLoadingIndicator()}</div>
        </div>
      )
    } else {
      return null
