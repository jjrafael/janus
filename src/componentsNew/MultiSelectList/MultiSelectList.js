...
class MultiSelectList extends React.Component {
  ...
  _onSelectionChange(selected, index) {
    const { disabled, onSelectionChange } = this.props

    if (disabled) return

    let { selectedItemsMap } = this.state
  ...
  _onSelectAllChange() {
    ...
    let { selectedItemsMap, allSelected } = this.state
  ...
  _renderListItems() {
    const { listItems, disabled } = this.props
  ...
  render() {
    const { disabled, listTitle } = this.props
    ...
    return (
      <div className={multiSelectListClass}>
        <div className={multiSelectListContainerClass}>{this._renderListTitle()}</div>
