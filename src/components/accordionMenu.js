    ...
    return (
      <div className={className}>
        <div onClick={this.onClick.bind(this)} className="accordion-heading">
          {this.state.iconOnLeft && <i className={`phxico ${icon} fleft`} />}
          {this.props.title}
          {!this.state.iconOnLeft && <i className={`phxico ${icon} fright`} />}
        </div>
        {this.state.isOpen && <div className="accordion-body">{this.props.children}</div>}
      </div>
    )
  }
}
