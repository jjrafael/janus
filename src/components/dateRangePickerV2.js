...
export default class DateRangePicker extends React.Component {
  ...
  handleInputChange(e) {
    ...
    if (moment(value, 'L', true).isValid()) {
      ...
      if (this.props.onDateChange) {
        this.props.onDateChange({ value, name, error: false })
      }
    } else {
      this.setState({ value }, this.showCurrentDate)
      if (this.props.onDateChange) {
        this.props.onDateChange({ value, name, error: true, errorMessage: 'Invalid date' })
      }
    }
  }
  ...
  handleDayClick(day, modifiers, e) {
    ...
    if (this.props.onDateChange) {
      this.props.onDateChange({ value: moment(day).format('L'), name: this.props.inputName })
    }
  }
  ...
  render() {
    ...
    return (
      <div
        className={`datepicker-container flex1-1 ${this.props.mainClass}`}
