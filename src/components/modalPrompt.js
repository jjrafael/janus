      ...
      >
        ...
        <form onSubmit={handleSubmit(onConfirm)}>
          ...
          <div className="button-group">
            <button className="btn btn-action" onClick={onCancel}>
              Cancel
            </button>
            <button
              className="btn btn-action btn-primary"
              disabled={!!defaultDisabled ? defaultDisabled : pristine || submitting}
            >
              {loading ? <i className="phxico phx-spinner phx-spin" title={loadingText} /> : 'Save Changes'}
            </button>
          </div>
        </form>
      </ModalWindow>
