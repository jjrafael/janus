...
class Outcome extends React.Component {
  ...
  shouldComponentUpdate(nextProps, nextState) {
    ...
    if (
      this.props.outcomeKeysTogglingVisibility.includes(key) !==
        nextProps.outcomeKeysTogglingVisibility.includes(key) ||
      (key === this.props.outcomeKey &&
        this.props.isFetchingOutcomePriceByLine !== nextProps.isFetchingOutcomePriceByLine) ||
      ...
      case 'feed':
        return (
          <span className="priceSourceIcon" key="priceSourceIcon">
            <i className="phxico icon-xsmall phx-rss push-right" />
          </span>
        )
        break
      case 'derived':
  ...
  _renderOutcomeRiskCells(outcomeData) {
    ...
    return riskDataConfig.columns.map((column, index) => {
      ...
      const isEditable =
        column.allowEdit && ['open', 'suspended', 'closed'].indexOf(this._getParentMarketStatus().toLowerCase()) > -1
      ...
      if (isEditingCell) {
        return (
          ...
          >
            <input
              ...
              onChange={e => {
                this._handlePriceInputChange(e.target.value)
              }}
        ...
        return (
          ...
          >
            {this.props.otherLinePrice && (
              ...
              >
                <div>
                  <span className="column-data clearfix block">{this._renderColumnData(columnData, column)}</span>
                </div>
              </Tooltip>
            )}
        ...
        return (
          <div className={className} key={index} style={{ minWidth: width, maxWidth: width }}>
            <span className="column-data clearfix block">{this._renderColumnData(columnData, column)}</span>
          </div>
        )
      }
  ...
  _renderVisibleIcon() {
    ...
    return (
      <a
        onClick={() => {
          this.props.onVisibilityIconClick({ ...this.DATA, id }, true)
        }}
      >
        <i className="phxico phx-eye icon-small" />
      </a>
    )
  }
  ...
  _renderHiddenFlag(hiddenFlag) {
    ...
    return (
      <a
        title={`Hidden ${daysAgo} - ${formattedTime}: ${reason}`}
        onClick={() => {
          this.props.onVisibilityIconClick({ ...this.DATA, id }, false)
        }}
      >
        <i className="phxico phx-eye-off icon-small" />
      </a>
    )
  }
  ...
  _renderResultIcon() {
    ...
    if (this._isVoided()) {
      return <i className="phxico phx-flag" title="Void" />
    }
    if (!result) {
      return null
    }
    switch (result.toUpperCase()) {
      case 'WIN':
        return <i className="phxico phx-trophy" title="Winner" />
      case 'LOSE':
        return <i className="phxico phx-sad" title="Placer" />
      case 'PLACE_2':
        return (
          <i className="phxico phx-custom icon-numeric" title="2nd placer">
            2
          </i>
        )
      case 'VOID':
  ...
  _renderAnalysisIcon() {
    return (
      ...
      >
        <span className="phxico phx-chart-bar" />
      </a>
    )
  }
  ...
  render() {
    ...
    const showSpreadOnDesc =
      riskDataConfig.visibleSpreadMarketTypeGroups.indexOf(parentMarket.marketTypeGroup) === -1 &&
      parentMarket.marketTypeGroup !== 'PROPOSITION'
    const isOpen =
      !parentMarket.flags ||
      (parentMarket.flags.indexOf('resulted') === -1 && parentMarket.flags.indexOf('settled') === -1)
    const parentMarketStatus = getMarketStatusFromFlags(parentMarket.flags)
    let spreadText = data.formattedSpread
    const rowClassName = `row outcome-row text-xmedium ${parentMarketStatus.toLowerCase()} ${this._isHidden(data)
      ? 'hidden-outcome'
      : ''}`
    return (
...
const mapStateToProps = (state, ownProps) => {
  ...
  let unsavedPriceChange =
    typeof riskDataChanges.unsavedOutcomePriceChanges[key] === 'undefined'
      ? null
      : riskDataChanges.unsavedOutcomePriceChanges[key]
