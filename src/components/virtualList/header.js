...
class Header extends React.Component {
  ...
  _renderColumns() {
    ...
    return this.props.columns.map((column, index) => {
      ...
      const isResizable =
        typeof column.resizable !== 'undefined' ? column.resizable : riskDataConfig.defaultResizableSetting
      ...
      if (column.desc.toLowerCase() === 'liability indicator') {
        return (
          <div key={index} className="column header" style={{ minWidth: width, maxWidth: width }}>
            <i className="phxico phx-exclamation" title={title} />
          </div>
        )
      } else if (column.desc.toLowerCase() === 'actions') {
        return (
          <div key={index} className="column header column-actions" style={{ minWidth: width, maxWidth: width }}>
            &nbsp;
          </div>
        )
      }
  ...
  render() {
    ...
    return <div className="row header-row">{this._renderColumns()}</div>
  }
}
