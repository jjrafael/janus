  ...
  _renderMarketSummaryCells(data) {
    return riskDataConfig.columns.map((column, index) => {
      ...
      if (column.desc === 'First Price') {
        columnHasUpdated = this._oldValue && this._oldValue[summaryKeys[0]] !== data[summaryKeys[0]]
        ...
        if (typeof column.formatData === 'function' && typeof data[dataKeys[0]] !== 'undefined') {
          columnHasUpdated = this._oldValue && this._oldValue[dataKeys[0]] !== data[dataKeys[0]]
      ...
      let className = `column tright column-${column.desc
        .toLowerCase()
        .split(' ')
        .join('-')
        .replace(/[^0-9a-z]/gi, '')}`
      return [
        <div style={{ minWidth: width, maxWidth: width }} className={`${className} ${updatedClass}`}>
          <span className="column-data clearfix block">{columnData}</span>
        </div>,
      ]
    })
  }
