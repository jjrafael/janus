...
class Event extends React.Component {
  ...
  _renderHighlightedDesc(desc, filter) {
    ...
    return indices.map((element, i) => {
      ...
      if (typeof indices[i + 1] === 'undefined' && element + filter.length < desc.length) {
        // last item in indices and there are words after
      ...
      return [<span>{pre}</span>, <span className="search-matching">{match}</span>, <span>{post}</span>]
    })
  }
  ...
  _renderEventMarketPeriods() {
    ...
    const periods = data.marketPeriods.map((period, index) => {
      return (
        <a className="push-right" key={index} onClick={this._handleMarketPeriodClick}>
          {period}
        </a>
      )
    })
    return <div className="event-market-periods">{[this._renderAnalysisButton(), ...periods]}</div>
  }
        ...
        <div className="column event">
          <div className="event-desc">
            <h4 className="text-bold text-large" title={data.desc}>
              <a onClick={eventId => onCollapseToggle(data.key, isCollapsed)} className="collapse-icon push-right">
                {isCollapsed && <i className="phxico phx-chevron-right icon-small" />}
                {!isCollapsed && <i className="phxico phx-chevron-down phx-icon-small" />}
              </a>
              {data.restricted && <span title="Restricted Event" className="push-right phxico phx-alert" />}
              {eventFilter.length < 2 && data.desc}
              {eventFilter.length >= 2 && this._renderHighlightedDesc(data.desc, eventFilter)}
            </h4>
          </div>
          <div className="market-periods">{this._renderEventMarketPeriods()}</div>
        </div>
      </div>
    )
  }
