...
class Market extends React.Component {
  ...
  shouldComponentUpdate(nextProps, nextState) {
    ...
    if (
      this.props.marketState.marketKeysChangingStatus.includes(key) !==
        nextProps.marketState.marketKeysChangingStatus.includes(key) ||
    ...
    const rowData = variableSpreadDataItem.spreadLiabilities.map((spreadLiability, index) => {
      ...
      return (
        <td className="tcenter" key={index}>
          <span className={className}>{formatNumber(spreadLiability.liability)}</span>
        </td>
      )
    })
    return (
  ...
  _renderSpacerColumns() {
    return riskDataConfig.columns.map((column, index) => {
      ...
      return <div key={index} style={{ minWidth: width, maxWidth: width }} className="column" />
    })
  }
        ...
        <div className="column market">
          <div className={disableActions ? 'market-desc' : 'market-desc enable-actions'}>
            <ul className="market-actions list-reset button-group">
              <li className="button">
                <a
                  className={disableActions ? 'disabled open' : 'open'}
                  onClick={
                    !disableActions
                      ? () => {
                          this.props.onMarketStatusIconClick(data.key, 'OPEN')
                        }
                      : () => {}
                  }
                >
                  <i className="phxico phx-play" />
                </a>
              </li>
              <li className="button">
                <a
                  className={disableActions ? 'disabled suspended' : 'suspended'}
                  onClick={
                    !disableActions
                      ? () => {
                          this.props.onMarketStatusIconClick(data.key, 'SUSPENDED')
                        }
                      : () => {}
                  }
                >
                  <i className="phxico phx-pause" />
                </a>
              </li>
              <li className="button">
                <a
                  className={disableActions ? 'disabled closed' : 'closed'}
                  onClick={
                    !disableActions
                      ? () => {
                          this.props.onMarketStatusIconClick(data.key, 'CLOSED')
                        }
                      : () => {}
                  }
                >
                  <i className="phxico phx-stop" />
                </a>
              </li>
              <li className="button">
                <a
                ...
                >
                  <i className="phxico phx-chart-bar" />
                </a>
              </li>
            </ul>
            <a
