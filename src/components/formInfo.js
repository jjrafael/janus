  ...
  <div className={'form-group ' + fieldLook} data-required={isRequired}>
    ...
    <div className={`form-group-control ${disabled}`}>
      <input
        ...
        className={formClass}
      />
      <span className="field-err">{touched && error && <span>{error}</span>}</span>
    </div>
  </div>
)
...
const renderSelectField = ({
  ...
  <div className={'form-group ' + fieldLook} data-required={isRequired}>
    {!removeLabel && <label className="form-group-label">{label} &nbsp; </label>}
    <div className={`form-group-control ${disabled}`}>
      <select {...input} disabled={disabled} defaultValue={defaultValue} value={value} className={formClass}>
        {!hidePlaceHolder && <option>{placeholder && <span>Select {placeholder}</span>}</option>}
...
const renderDateField = ({
  ...
  <div className={'form-group ' + fieldLook} data-required={isRequired}>
    <label className="form-group-label" style={{ lineHeight: '20px' }}>
      {label}
    </label>
    <DateRangePicker
      disabled={disabled}
      ...
      placeholder={placeholder}
    >
      <span className="field-err">{error && <span>{error}</span>}</span>
    </DateRangePicker>
  </div>
)
    ...
    return (
      <div>
        {formInfo.map((info, index) => {
          return (
            <FormElement key={index} title={formHead} formWrapperClass={className}>
              {info.data.map((data, index) => {
                return (
                  <Field
                    ...
                    name={data.name}
                    component={this._rendererComponent(data.type)}
                    type={data.type}
                    ...
                    lists={data.lists}
                    hidePlaceHolder={data.hidePlaceHolder}
                    validate={data.validate}
                    isRequired={data.isRequired}
                    validate={data.validate}
                    value={data.value}
                    showDatePickerOnFocus={data.showDatePickerOnFocus}
                    onDateChange={data.onDateChange}
                    mainClass={data.mainClass}
                    datePickerClass={data.datePickerClass}
